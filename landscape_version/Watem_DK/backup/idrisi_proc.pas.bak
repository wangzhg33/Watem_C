...
uses
  Classes, SysUtils,Rdata,Gdata, Math;
  ...
  setlength(testarray,ncol+1);
   for i:= 1 to nrow do
    begin
     ...
     testarray[j]:=Z[i,j+1];
     if MaxValue(testarray)>MAXZ then MAXZ:=MaxValue(testarray);
     if MinValue(testarray)<MINZ then MINZ:=MinValue(testarray);
     blockwrite(outputf, testarray[0],ncol);
    end;
  testarray:=NIL;
...
procedure writeGIdrisifile(pncol,pnrow:Integer;filename:string;z: Graster);  { TODO -oKVO -cupdate write idrisifiles for integer datatypes :  }
var
dumstr:string;
i,j : integer;
outputdoc : textfile;
